...
resource "libvirt_network" "network" {
  ...
  dns {
    ...
    # use Ansible instead and amend manually, see ('../ansible/         ')
    # dynamic "hosts" {
    #     for_each = var.hosts
    #     content {
...
resource "null_resource" "update_inventory" {
  triggers = {
    ...
    host_ids = join(" ", values(merge({ bootstrap = libvirt_domain.host_bootstrap }, libvirt_domain.host_master, libvirt_domain.host_worker))[*].id)
  }
  provisioner "local-exec" {
    # recreate ansible inventory
    command = "echo '${templatefile("${path.module}/templates/inventory.tpl", { hosts = merge({ bootstrap = libvirt_domain.host_bootstrap }, libvirt_domain.host_master, libvirt_domain.host_worker) })}' > ../ansible/inventory"
  }
  provisioner "local-exec" {
    # recreate mapping of qemu VM id to hostnames
    command = "echo '${templatefile("${path.module}/templates/qemu-config.yml.tpl", { hosts = merge({ bootstrap = libvirt_domain.host_bootstrap }, libvirt_domain.host_master, libvirt_domain.host_worker) })}' > ../ansible/qemu-config.yml"
  }
}
